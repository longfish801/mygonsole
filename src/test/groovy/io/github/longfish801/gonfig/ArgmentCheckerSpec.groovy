...
class ArgmentCheckerSpec extends Specification {
	...
	def 'checkExistFile'(){
		...
		then:
		exc = thrown(IllegalArgumentException)
		exc.message == /引数は存在しないファイルです。arg=[src\test\resources\io\github\longfish801\gonfig\ArgmentCheckerSpec\noSuchFile.txt]/
	}
	...
	def 'checkExistDirectory'(){
		...
		then:
		exc = thrown(IllegalArgumentException)
		exc.message == /引数は存在しないフォルダです。arg=[src\test\resources\io\github\longfish801\gonfig\ArgmentCheckerSpec\test.txt]/
	}
}
